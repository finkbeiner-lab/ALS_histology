...
//File folder = new File('file:/Volumes/Finkbeiner-Steve/work/data/npsad_data/vivek/QuPath')
//def project = ""
...
//File new_file = new File('file:/Volumes/Finkbeiner-Steve/work/data/npsad_data/vivek/QuPath/XE07-049_1_AmyB_1/project.qpproj')
//File new_file = new File('/gladstone/finkbeiner/steve/work/data/npsad_data/vivek/QuPath/XE07-049_1_AmyB_1/project.qpproj')
...
image_val = image_list[9]

 def image_data = image_val.readImageData()
path = image_val.getImageName()
 describe(image_data)
 
 PathObjectHierarchy heirarchy = image_data.getHierarchy()
 Collection annotations = heirarchy.getAnnotationObjects()
 print(annotations)
 def annotationsLength = annotations.size()

 println "Number of annotations: $annotationsLength"
...
for (item in annotations) {
    ...
    def temp_results = [:]
    temp_results["label"] = "Null"
    temp_results["filename"] = filename
    temp_results["roiName"] = roiName
    if (roiName=="Ellipse"){
    temp_results["label"] = "tdp43"
    }
    if (roiName=="Rectangle"){
    temp_results["label"] = "glial"
    }
...
savepath = "/Volumes/Finkbeiner-Steve/work/data/ALS/annotations/sals-lumbar/" + filename + ".json"
print(savepath)
